package com.yc.damai.dao;

import java.util.List;

import com.yc.damai.po.DmAddress;
import com.yc.damai.util.DBHelper;

public class AddressDao {
	 //通过id查用户地址信息
	 public List<?> querydft(String uid) {
		 String sql="select * from dm_address where dft=1 and uid=?";
		 DBHelper dbh=new DBHelper(); 
		return dbh.query(sql,uid); 
	 }
	
	
		public int insert(DmAddress dp,String uid) {
			String sql = "insert into dm_address values(null,?,?,?,?,1,now())";
			return	new DBHelper().update(sql,uid
					,dp.getAddr()
					,dp.getPhone()
					,dp.getName()
					);
		}

		public int update(DmAddress dp,String uid) {
			String sql = "update dm_address set addr=?,phone=?,name=?,dft=1  where uid=?";
			return new DBHelper().update(sql
					,dp.getAddr()
					,dp.getPhone()
					,dp.getName()
					,uid);
		}
		
		
		 //用户地址信息
		 public List<?> addrinfo(String uid) {
			 String sql="select * from dm_address where uid=?";
			 DBHelper dbh=new DBHelper(); 
			return dbh.query(sql,uid); 
		 }
		 //通过id查用户地址信息
		 public List<?> addrinfo1(String uid) {
			 String sql="select * from dm_address where id=?";
			 DBHelper dbh=new DBHelper(); 
			return dbh.query(sql,uid); 
		 }
		 //通过id查用户地址信息
		 public int modifyinfo1(String addr,String phone,String name,int dft,String id) {
			 String sql="update dm_address set addr=?,phone=?,name=?,dft=? where id=?";
			 DBHelper dbh=new DBHelper(); 
			return dbh.update(sql,addr,phone,name,dft,id); 
		 }
		 //添加地址
		 public int add(String uid,String addr,String phone,String name,int dft) {
			 String sql="insert into dm_address(uid,addr,phone,name,dft) "
			 		+ "value(?,?,?,?,?);";
			 DBHelper dbh=new DBHelper(); 
			return dbh.update(sql,uid,addr,phone,name,dft); 
		 }
		//删除地址
			 public int del(String id) {
				 String sql="delete from dm_address where id=?";
				 DBHelper dbh=new DBHelper(); 
				return dbh.update(sql,id); 
			 }
		//改变默认地址
			 public int modifydft(String uid) {
				 String sql="update dm_address set dft=0 where uid=?";
				 DBHelper dbh=new DBHelper(); 
				return dbh.update(sql,uid); 
			 } 
		
		
		
		
}
